(() => {
  ...
  function ensurePanel(){
    ...
    node.innerHTML = `
      <section class="ai-chat-panel" role="dialog" aria-modal="true" aria-labelledby="aiChatTitle">
        <header class="ai-chat-head">
          ...
          <div class="ai-chat-title"><small>AI·CLO APMaths</small><h2 id="aiChatTitle">Hỏi AI·CLO</h2></div>
        ...
        <div class="ai-chat-body">
          ...
          <div class="ai-chat-messages" aria-live="polite">
            <div class="ai-chat-message ai">Xin chào! Bạn muốn hỏi gì về AI·CLO APMaths?</div>
          </div>
    ...
    $('.ai-chat-form',node).addEventListener('submit',e=>{
      e.preventDefault();
      const input=$('.ai-chat-input',node), text=input.value.trim();
      if(!text) return;
      input.value='';
      void ask(text);
    });
  ...
  function setSending(value){
    sending=value;
    const send=$('.ai-chat-send'), input=$('.ai-chat-input');

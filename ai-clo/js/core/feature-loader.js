/* AI-CLO PTITHCM V12.6.51 — on-demand loader for independent utilities only.
...
const AI_CLONE='js/questions/ai-clone.js?v=12.6.50';
const COURSE_AI_GUIDE='js/courses/ai-guide.js?v=12.6.50';
const MOODLE_EXPORT='js/questions/moodle-export.js?v=12.6.51';
...
const lazyAiGenerate=async(...args)=>{await loadAiReviewFlow();await loadScript('js/ai/generator.js?v=12.6.50');const fn=window.aiGenerateForm;if(typeof fn!=='function'||fn===lazyAiGenerate)throw new Error('Không khởi tạo được chức năng tạo câu hỏi AI.');return fn(...args)};window.aiGenerateForm=lazyAiGenerate;
...
async function bootstrapIndependentFeatures(){installNavigationGate();for(const [src,label] of [[AI_CLONE,'AI clone'],[COURSE_AI_GUIDE,'course AI guide'],[MOODLE_EXPORT,'Moodle export']]){try{await loadScript(src)}catch(e){console.error(`AI-CLO ${label} module load failed`,e)}}}

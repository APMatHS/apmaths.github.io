...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  if(action==="resolve"){
   const {data:packages,error:pe}=await admin.from("practice_packages").select("id,name,chapter_ids,topic_ids,draw_mode,matrix_scope,question_count,include_answers,sort_order").eq("subject_id",subject.id).eq("is_enabled",true).order("sort_order").order("created_at");if(pe)throw pe;
   const chapterIds=[...new Set((packages||[]).flatMap((p:any)=>p.chapter_ids||[]))];
   const topicIds=[...new Set((packages||[]).flatMap((p:any)=>p.topic_ids||[]))];
   let chapterNames=new Map<string,string>(),topicNames=new Map<string,string>();
   if(chapterIds.length){const {data:chs,error:ce}=await admin.from("chapters").select("id,name").in("id",chapterIds);if(ce)throw ce;chapterNames=new Map((chs||[]).map((c:any)=>[c.id,c.name]));}
   if(topicIds.length){const {data:ts,error:te}=await admin.from("topics").select("id,name").in("id",topicIds);if(te)throw te;topicNames=new Map((ts||[]).map((t:any)=>[t.id,t.name]));}
   return json({success:true,subject:{name:subject.name,semester:subject.semester,academic_year:subject.academic_year},practice:{title:config.title||`Đề ôn tập – ${subject.name}`,allow_unlimited_redraw:config.allow_unlimited_redraw},packages:(packages||[]).map((p:any)=>({id:p.id,name:p.name,draw_mode:p.draw_mode,matrix_scope:p.matrix_scope||"chapter",question_count:p.question_count,include_answers:p.include_answers,chapters:(p.chapter_ids||[]).map((id:string)=>chapterNames.get(id)).filter(Boolean),topics:(p.topic_ids||[]).map((id:string)=>topicNames.get(id)).filter(Boolean)}))});
  }
  ...
  const topicIds=Array.isArray(pkg.topic_ids)?pkg.topic_ids:[];
  let poolQuery=admin.from("questions").select("id,content,correct_answer,explanation,chapter_id,topic_id,clo_id,display_code").eq("question_bank_id",subject.question_bank_id).eq("question_scope","practice").eq("approval_status","approved").eq("status","active").in("chapter_id",pkg.chapter_ids).order("id");
  if(topicIds.length)poolQuery=poolQuery.in("topic_id",topicIds);
  const {data:poolRows,error:poolError}=await poolQuery;if(poolError)throw poolError;const pool=poolRows||[];if(!pool.length)return json({success:false,error:"Phạm vi của gói này chưa có câu hỏi luyện tập đã duyệt."},409);

  if(!selectedIds){
   const picked:string[]=[];
   if(pkg.draw_mode==="matrix"){
    const matrixScope=clean(pkg.matrix_scope)==="topic"?"topic":"chapter";
    const requested=(Array.isArray(pkg.matrix)?pkg.matrix:[]).map((x:any)=>({chapter_id:clean(x.chapter_id),topic_id:clean(x.topic_id),clo_id:clean(x.clo_id),count:Number(x.count||0)})).filter((x:any)=>x.clo_id&&Number.isInteger(x.count)&&x.count>0);
    for(const cell of requested){
     const candidates=matrixScope==="topic"?pool.filter((q:any)=>q.topic_id===cell.topic_id&&q.clo_id===cell.clo_id):pool.filter((q:any)=>q.chapter_id===cell.chapter_id&&q.clo_id===cell.clo_id);
     if(candidates.length<cell.count)return json({success:false,error:`Ngân hàng không đủ câu cho ma trận đã cấu hình (${cell.count} cần, ${candidates.length} có).`},409);
     const key=matrixScope==="topic"?cell.topic_id:cell.chapter_id;
     picked.push(...shuffled(candidates,`${seed}|${pkg.id}|${matrixScope}|${key}|${cell.clo_id}`).slice(0,cell.count).map((q:any)=>q.id));
    }
    if(!picked.length)return json({success:false,error:"Gói này chưa có ma trận CLO hợp lệ."},409);
   }else{
    const count=Number(pkg.question_count||20);if(count>pool.length)return json({success:false,error:`Gói này cần ${count} câu nhưng hiện chỉ có ${pool.length} câu đủ điều kiện.`},409);picked.push(...shuffled(pool,`${seed}|${pkg.id}|COUNT`).slice(0,count).map((q:any)=>q.id));
   }
   selectedIds=picked;
   const {error:le}=await admin.from("practice_draws").insert({subject_id:subject.id,package_id:pkg.id,seed,selected_question_ids:selectedIds});if(le&&!/duplicate/i.test(le.message||""))throw le;
  }

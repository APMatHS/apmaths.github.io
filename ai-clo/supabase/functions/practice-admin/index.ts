...
async function availability(questionBankId:string){
 const {data,error}=await admin.from("questions").select("id,chapter_id,topic_id,clo_id").eq("question_bank_id",questionBankId).eq("question_scope","practice").eq("approval_status","approved").eq("status","active");
 if(error)throw error;
 const chapterClo:Record<string,number>={},topicClo:Record<string,number>={},chapterCounts:Record<string,number>={},topicCounts:Record<string,number>={};
 for(const q of data||[]){
  chapterClo[`${q.chapter_id}|${q.clo_id}`]=(chapterClo[`${q.chapter_id}|${q.clo_id}`]||0)+1;
  topicClo[`${q.topic_id}|${q.clo_id}`]=(topicClo[`${q.topic_id}|${q.clo_id}`]||0)+1;
  chapterCounts[q.chapter_id]=(chapterCounts[q.chapter_id]||0)+1;
  topicCounts[q.topic_id]=(topicCounts[q.topic_id]||0)+1;
 }
 return{rows:data||[],chapterClo,topicClo,chapterCounts,topicCounts};
}
...
Deno.serve(async(req)=>{
 ...
 try{
  ...
  const subjectId=clean(body.subject_id);if(!subjectId)return reply({success:false,error:"Thiếu subject_id."},400);
  ...
  if(action==="detail"){
   const [cr,chr,tr,clr,pr,av]=await Promise.all([
    ...
    admin.from("topics").select("id,chapter_id,name,order_index").eq("question_bank_id",subject.question_bank_id).order("order_index"),
   ...
   if(cr.error||chr.error||tr.error||clr.error||pr.error)throw cr.error||chr.error||tr.error||clr.error||pr.error;
   ...
   const [{count:total},{count:week}]=await Promise.all([
    ...
    admin.from("practice_draws").select("id",{count:"exact",head:true}).eq("subject_id",subjectId).gte("drawn_at",seven)
   ]);
   return reply({success:true,subject,config:cr.data||null,chapters:chr.data||[],topics:tr.data||[],clos:clr.data||[],packages:pr.data||[],availability:av.chapterClo,topic_availability:av.topicClo,chapter_availability:av.chapterCounts,topic_counts:av.topicCounts,total_available:av.rows.length,stats:{total_draws:total||0,week_draws:week||0}});
  }
  ...
  if(action==="rotate_code"){
   ...
   const r=existing?await admin.from("practice_configs").update({access_code:code,updated_by:profile.id,updated_at:now}).eq("subject_id",subjectId).select("*").single():await admin.from("practice_configs").insert({subject_id:subjectId,access_code:code,is_enabled:false,updated_by:profile.id}).select("*").single();
   if(r.error)throw r.error;return reply({success:true,config:r.data});
  }
  ...
  if(action==="save_package"){
   ...
   const [chr,tr,clr,av]=await Promise.all([
    admin.from("chapters").select("id").eq("question_bank_id",subject.question_bank_id),
    admin.from("topics").select("id,chapter_id").eq("question_bank_id",subject.question_bank_id),
    admin.from("clos").select("id").eq("question_bank_id",subject.question_bank_id),
    availability(subject.question_bank_id)
   ]);
   if(chr.error||tr.error||clr.error)throw chr.error||tr.error||clr.error;
   const validCh=new Set((chr.data||[]).map((x:any)=>x.id)),validCl=new Set((clr.data||[]).map((x:any)=>x.id)),topicChapter=new Map((tr.data||[]).map((x:any)=>[x.id,x.chapter_id]));
   const chapterIds=[...new Set((Array.isArray(body.chapter_ids)?body.chapter_ids:[]).map(clean).filter(Boolean))];
   if(!chapterIds.length||chapterIds.some(id=>!validCh.has(id)))return reply({success:false,error:"Hãy chọn ít nhất một chương hợp lệ."},400);

   let topicIds=[...new Set((Array.isArray(body.topic_ids)?body.topic_ids:[]).map(clean).filter(Boolean))];
   if(!topicIds.length)topicIds=(tr.data||[]).filter((t:any)=>chapterIds.includes(t.chapter_id)).map((t:any)=>t.id);
   if(!topicIds.length||topicIds.some(id=>!topicChapter.has(id)||!chapterIds.includes(topicChapter.get(id))))return reply({success:false,error:"Phạm vi Mục không hợp lệ."},400);
   for(const ch of chapterIds){if(!topicIds.some(id=>topicChapter.get(id)===ch))return reply({success:false,error:"Mỗi Chương đã chọn cần có ít nhất một Mục được chọn."},400);}

   const mode=clean(body.draw_mode)==="matrix"?"matrix":"count";
   const matrixScope=clean(body.matrix_scope)==="topic"?"topic":"chapter";
   let questionCount=Number(body.question_count||20),matrix:any[]=[];
   const scopedRows=av.rows.filter((q:any)=>chapterIds.includes(q.chapter_id)&&topicIds.includes(q.topic_id));
   ...
   if(mode==="count"){
    ...
    if(questionCount>scopedRows.length)return reply({success:false,error:`Phạm vi đã chọn chỉ có ${scopedRows.length} câu luyện tập đã duyệt.`},409);
   }else{
    const raw=Array.isArray(body.matrix)?body.matrix:[];
    for(const x of raw){
     const clo_id=clean(x.clo_id),count=Number(x.count||0);if(!count)continue;if(!validCl.has(clo_id)||!Number.isInteger(count)||count<0||count>200)return reply({success:false,error:"Ma trận CLO không hợp lệ."},400);
     if(matrixScope==="chapter"){
      const chapter_id=clean(x.chapter_id);if(!chapterIds.includes(chapter_id))return reply({success:false,error:"Ma trận Chương × CLO không hợp lệ."},400);
      const have=scopedRows.filter((q:any)=>q.chapter_id===chapter_id&&q.clo_id===clo_id).length;
      if(count>have)return reply({success:false,error:`Một ô Chương × CLO yêu cầu ${count} câu nhưng chỉ có ${have} câu trong các Mục đã chọn.`},409);
      matrix.push({chapter_id,clo_id,count});
     }else{
      const topic_id=clean(x.topic_id);if(!topicIds.includes(topic_id))return reply({success:false,error:"Ma trận Mục × CLO không hợp lệ."},400);
      const have=scopedRows.filter((q:any)=>q.topic_id===topic_id&&q.clo_id===clo_id).length;
      if(count>have)return reply({success:false,error:`Một ô Mục × CLO yêu cầu ${count} câu nhưng chỉ có ${have} câu.`},409);
      matrix.push({topic_id,clo_id,count});
     }
    }
   ...
   const payload={subject_id:subjectId,name,is_enabled:asBool(body.is_enabled,true),chapter_ids:chapterIds,topic_ids:topicIds,draw_mode:mode,matrix_scope:matrixScope,question_count:questionCount,matrix,include_answers:asBool(body.include_answers,false),sort_order:Number.isInteger(Number(body.sort_order))?Number(body.sort_order):0,updated_at:new Date().toISOString()};
   let r;
   if(packageId){const own=await admin.from("practice_packages").select("id").eq("id",packageId).eq("subject_id",subjectId).maybeSingle();if(!own.data)return reply({success:false,error:"Không tìm thấy gói ôn tập."},404);r=await admin.from("practice_packages").update(payload).eq("id",packageId).eq("subject_id",subjectId).select("*").single();}
   else r=await admin.from("practice_packages").insert(payload).select("*").single();
   if(r.error)throw r.error;return reply({success:true,package:r.data});
  }

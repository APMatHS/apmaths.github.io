...
function updateVoiceList() {
    ...
    const exists =
        voices.some(
            voice => voice.value === previousVoice
        );
    if (exists) {
...
function updateModelList() {
    ...
    const exists =
        models.some(
            model => model.value === previousModel
        );
    if (exists) {
...
function setAudioSource(blob) {
    ...
    downloadButton.href =
        currentAudioUrl;
    if (engineSelect.value === "chirp") {
        downloadButton.download = "chirp-3-tts.mp3";
    } else if (engineSelect.value === "cloud") {
        downloadButton.download = "cloud-tts.mp3";
    } else {
